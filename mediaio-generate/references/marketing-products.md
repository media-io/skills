...
```bash
ID=$(mediaio marketing-studio products fetch --url https://shop.example.com/sneakers --wait --json | jq -r .id)
```
...
```bash
mediaio marketing-studio webproducts fetch --url https://apps.apple.com/... --wait
```
...
```bash
A=$(mediaio upload create shoe1.png)
B=$(mediaio upload create shoe2.png)
mediaio marketing-studio products create \
  --title "AeroRun Pro" \
...
```bash
mediaio marketing-studio webproducts create \
...
```bash
mediaio marketing-studio products list
mediaio marketing-studio products list --json
mediaio marketing-studio webproducts list
```
